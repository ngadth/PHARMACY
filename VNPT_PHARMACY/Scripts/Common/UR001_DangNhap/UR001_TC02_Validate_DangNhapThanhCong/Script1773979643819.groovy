...
def testCases = [
	[
		...
		logo: findTestObject('Common/logo_vnpt')
		],
...
for (def tc : testCases) {
    ...
    // verify lỗi
    WebUI.verifyElementVisible(tc.logo)

    WebUI.delay(2)

    WebUI.callTestCase(findTestCase('Common/TC03_DangXuat'), [:], FailureHandling.STOP_ON_FAILURE)
}
