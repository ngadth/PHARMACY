...
WebUI.sendKeys(ghiChuField, 'Nhập kho hàng hóa - Test Thủ kho nhập')

WebUI.click(findTestObject('XuatKhoBanHang/button_lastDynamicLocators', [('buttonName') : 'Lưu']))

WebUI.delay(3)

WebUI.verifyElementPresent(findTestObject('Admin/Common/text_hDynamicLocators', [('text') : 'Lập phiếu nhập kho']), 3, FailureHandling.STOP_ON_FAILURE)

String thuKhoValue = "<script>alert(123)</script>"

WebUI.sendKeys(findTestObject('QuanLyKho/input_labelDynamicLocators', [('labelValue') : 'Thủ kho nhập ']), thuKhoValue)
...
String savedThuKhoValue = WebUI.getAttribute(findTestObject('QuanLyKho/input_labelDynamicLocators', [('labelValue') : 'Thủ kho nhập ']), 'value')
