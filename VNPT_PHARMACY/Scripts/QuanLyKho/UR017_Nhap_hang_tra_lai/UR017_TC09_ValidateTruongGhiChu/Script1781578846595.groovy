...
String ghiChuMixed = 'Ghi chú 123'

TestObject ghiChuObject = new TestObject('ghiChuField')
ghiChuObject.addProperty('xpath', ConditionType.EQUALS, "(//label[contains(normalize-space(.),'Ghi chú')]/following-sibling::div[1]//input[@type='text'])[2]")

WebUI.waitForElementVisible(ghiChuObject, 10)
WebUI.waitForElementClickable(ghiChuObject, 10)
WebUI.scrollToElement(ghiChuObject, 5)
WebUI.click(ghiChuObject, FailureHandling.OPTIONAL)

String ghiChuValue = WebUI.getAttribute(ghiChuObject, 'value')

if (ghiChuValue != null && ghiChuValue.trim() != '') {
	try {
		WebUI.clearText(ghiChuObject)
	} catch (Exception ex) {
		def ghiChuElement = WebUI.findWebElement(ghiChuObject, 10)
		WebUI.executeJavaScript('arguments[0].value = ""; arguments[0].dispatchEvent(new Event("input", { bubbles: true })); arguments[0].dispatchEvent(new Event("change", { bubbles: true }));',
			Arrays.asList(ghiChuElement))
	}
}

String clearedGhiChuValue = WebUI.getAttribute(ghiChuObject, 'value')
WebUI.verifyEqual(clearedGhiChuValue == null ? '' : clearedGhiChuValue.trim(), '', FailureHandling.STOP_ON_FAILURE)

try {
	WebUI.setText(ghiChuObject, ghiChuMixed)
} catch (Exception ex) {
	def ghiChuElement = WebUI.findWebElement(ghiChuObject, 10)
	WebUI.executeJavaScript('arguments[0].value = arguments[1]; arguments[0].dispatchEvent(new Event("input", { bubbles: true })); arguments[0].dispatchEvent(new Event("change", { bubbles: true }));',
		Arrays.asList(ghiChuElement, ghiChuMixed))
}

String setGhiChuValue = WebUI.getAttribute(ghiChuObject, 'value')

WebUI.verifyEqual(setGhiChuValue, ghiChuMixed, FailureHandling.STOP_ON_FAILURE)
