...
// 1. Send request
def response = WS.sendRequest(findTestObject('API/API_CungCapCho_HKD/DongBoChungTuMuaHang_get_buying_order', [('url') : GlobalVariable.url_hkd]))
...
rsList.eachWithIndex({ def item, def idx ->
        def soPhieuChiRaw = item.SO_PHIEU_CHI

// bỏ qua nếu null
if (soPhieuChiRaw != null) {
    def soPhieuChi = soPhieuChiRaw.toString().trim()

    // chỉ check khi có giá trị
    if (soPhieuChi) {
