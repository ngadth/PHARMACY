...
// Gửi request
response = WS.sendRequest(findTestObject('API/API_FOR_MOBILEAPP/QuanLyDonDatHangNCC/CapNhatDonDatHangNCC'))
...
// Tách phần chữ (TRUE)
def textTRUEUD = returnValue.find(/[A-Za-z]+/)

IDUD = returnValue.find(/\d+$/)

println("Phần chữ là: " + textTRUEUD)

println("Phần số là: " + IDUD)

WebUI.verifyMatch(textTRUEUD, status, true)
