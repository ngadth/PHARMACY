...
//send request of api get buying order
RequestObject req = findTestObject('API/API_TICHHOP_HKD/ChungTuNhapHangTraLai')

def resp = WS.sendRequest(findTestObject('API/API_TICHHOP_HKD/ChungTuNhapHangTraLai', [('access_token') : GlobalVariable.access_token]))
...
def respFmt = new DateTimeFormatterBuilder().appendPattern('dd/MM/yyyy HH:mm').optionalStart().appendPattern(':ss').optionalEnd().toFormatter()

def bad = rows.findAll({ def r ->
        def raw = r.NGAY_CHUNG_TU.toString().trim()

        if (!(raw)) {
            return true
        }
        
        def d = LocalDateTime.parse(raw, respFmt).toLocalDate()
